...
rotation_center = np.array([hatch_distance * num_paths / 2, extrusion_depth / 2])
...
for j in range(num_layers):
    ...
    for i in range(num_paths):
        ...
        center_y = 0
        center_z = j * layer_spacing

        # Half-ellipse with flat top
        x_arc = width/2 * np.cos(theta)
        z_arc = height * np.sin(theta)
        ...
        if x_cross:
            ...
            ax.fill(x_cross[sorted_indices], z_cross[sorted_indices],
...
ax.set_xlim(8*hatch_distance, 8*hatch_distance+500)
ax.set_ylim(0, 500)
plt.tight_layout()

# Add 100 µm scale bar at lower-right corner
scalebar_length = 100  # µm
x_start = 8*hatch_distance + 350  # horizontal position
